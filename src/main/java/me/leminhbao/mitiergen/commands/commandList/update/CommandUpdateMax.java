...
@AllArgsConstructor
public class CommandUpdateMax {
    private final MiTierGen plugin;

    public void onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        if (!sender.hasPermission("mtg.update.max") || !sender.hasPermission("mtg.update") || !sender.hasPermission("mtg.admin")) {
            sender.sendMessage(plugin.getPrefix() + "You do not have permission to use this command.");
            return;
        }

        if (args.length != 5) {
            sender.sendMessage(plugin.getPrefix() + "Usage: /mitiergen update_stat_max <tier name> <weapon name> <stat name> <new value>");
            return;
        }
    ...
    private boolean updateStat(String tierID, String weaponID, String statID, String newValue) {
        try {
            return plugin.getMiTierConfig().updateStatMax(tierID, weaponID, statID, Double.parseDouble(newValue));
